...
func PrepareEthTx(
	...
	baseDenom := appEthermint.EvmKeeper.GetParams(ctx).EvmDenom
...
func CreateEthTx(
	...
	baseFeeRes, err := appEthermint.EvmKeeper.BaseFee(ctx, &evmtypes.QueryBaseFeeRequest{})
	if err != nil {
	...
	nonce := appEthermint.EvmKeeper.GetNonce(ctx, fromAddr) + uint64(nonceIncrement) //nolint:gosec // G115

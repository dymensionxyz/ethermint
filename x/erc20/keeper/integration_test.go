	...
	Describe("Submitting a token pair proposal through governance", func() {
		Context("with existing coins", func() {
			BeforeEach(func() {
				...
				coins := sdk.NewCoins(
					sdk.NewCoin(teststypes.BaseDenom, fundsAmt),
			...
			Describe("for a single Cosmos Coin", func() {
				BeforeEach(func() {
					...
					_, err = testutil.Delegate(s.ctx, s.app, privKey, sdk.NewCoin(teststypes.BaseDenom, sdk.NewInt(500000000000000000)), s.validator)
					s.Require().NoError(err)
	...
	Describe("Converting", func() {
		Context("with a registered coin", func() {
			BeforeEach(func() {
				...
				err := testutil.FundAccount(s.ctx, s.app.BankKeeper, accAddr, sdk.NewCoins(sdk.NewCoin(teststypes.BaseDenom, fundsAmt)))
				s.Require().NoError(err)

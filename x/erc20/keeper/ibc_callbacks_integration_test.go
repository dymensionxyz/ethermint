...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	evmosMeta := banktypes.Metadata{
		...
		Base:        teststypes.BaseDenom,
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    teststypes.AphotonDenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    teststypes.BaseDenom,
		Symbol:  erc20Symbol,
		Display: teststypes.AphotonDenomtrace.BaseDenom,
	}
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert aphoton", func() {
			// Register 'aphoton' coin in ERC-20 keeper to validate it is not converting the coins when receiving 'aphoton' thru IBC
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.EvmosChain.GetContext(), evmosMeta)
			s.Require().NoError(err)

			aphotonInitialBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.BaseDenom)

			// 1. Send aphoton from Evmos to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.EvmosChain, teststypes.BaseDenom, amount, receiver, sender, 1, "")

			aphotonAfterBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.BaseDenom)
			s.Require().Equal(aphotonInitialBalance.Amount.Sub(math.NewInt(amount)).Sub(sendAndReceiveMsgFee), aphotonAfterBalance.Amount)

			// check ibc aphoton coins balance on Osmosis
			aphotonIBCBalanceBefore := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AphotonIbcdenom)
			s.Require().Equal(amount, aphotonIBCBalanceBefore.Amount.Int64())

			// 2. Send aphoton IBC coins from Osmosis to Evmos
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.AphotonDenomtrace.Path, teststypes.AphotonDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, teststypes.AphotonIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc aphoton coins balance on Osmosis - should be zero
			aphotonIBCSenderFinalBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AphotonIbcdenom)
			s.Require().Equal(int64(0), aphotonIBCSenderFinalBalance.Amount.Int64())

			// check aphoton balance after transfer - should be equal to initial balance
			aphotonFinalBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.BaseDenom)

			totalFees := sendBackCoinsFee.Add(sendAndReceiveMsgFee)
			s.Require().Equal(aphotonInitialBalance.Amount.Sub(totalFees), aphotonFinalBalance.Amount)

			// check IBC Coin balance - should be zero
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.AphotonIbcdenom)
